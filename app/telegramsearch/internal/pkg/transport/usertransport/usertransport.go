...
func extractEntityText(text string, offset, length int) string {
	...
	if offset+length > len(utf16Codes) {
		length = len(utf16Codes) - offset
	}
	if length <= 0 {
		return ""
	}

	entityCodeUnits := utf16Codes[offset : offset+length]
...
func reconstructFullTextEntities(fullText string, entities []map[string]interface{}) []map[string]interface{} {
	...
	for _, entity := range entities {
		offset, ok := entity["offset"].(int)
		if !ok {
			continue
		}
		length, ok := entity["length"].(int)
		if !ok {
			continue
		}

		if offset > lastEnd {
			plainText := string(runes[lastEnd:offset])
			if plainText != "" {
	...
	if lastEnd < len(runes) {
		plainText := string(runes[lastEnd:])
		if plainText != "" {
			result = append(result, map[string]interface{}{
				"type": "plain",
				"text": plainText,
...
func writeMessagesToDatabase(ctx context.Context, storage *postgres.Storage, messages []*tg.Message, chatIdentifier string) error {
	fmt.Printf("Saving %d messages to database for chat: %s\n", len(messages), chatIdentifier)
	...
	for _, msg := range messages {
		...
		// TODO: что подумают про это друзья? 11
		...
		fmt.Printf("Saved message %d with DB ID %d\n", msg.ID, resp.MessageID)
	}

	fmt.Printf("Successfully saved %d messages to database\n", len(messages))
	return nil
}
...
func ReadChatMessages(ctx context.Context, cfg Config, postgresConfig postgres.Config) error {
	...
	waiter := floodwait.NewWaiter().WithCallback(func(ctx context.Context, wait floodwait.FloodWait) {
		fmt.Printf("Got FLOOD_WAIT. Will retry after %v\n", wait.Duration)
	})
	...
	return waiter.Run(ctx, func(ctx context.Context) error {
		...
		if err := client.Run(ctx, func(ctx context.Context) error {
			...
			for i, chatIDSecret := range cfg.ChatIDs {
				...
				fmt.Printf("Processing chat %d out of %d (ID: %d)\n", i+1, len(cfg.ChatIDs), chatID)
				...
				fmt.Printf("Chat %d: processed %d messages\n", chatID, messages)
			}
...
func readMessagesFromChannel(ctx context.Context, client *telegram.Client, storage *postgres.Storage, channelID int64, accessHash int64, chatIdentifier string) (int, error) {
	totalProcessed := 0 // TODO: наверное лучше возвращать lastid измененный, idk 17
	...
	for {
		...
		fmt.Printf("%d messages, last ID: %d, saved to bd: %d\n", len(processed), processed[len(processed)-1].ID, len(processed))
